...
def referral(
    data_file: Path = typer.Argument(
        ...,
        help="Path to the referral data file (.xls/.xlsx/.csv).",
    ),
    config: Path = typer.Option(
        DEFAULT_CONFIG_PATH, "--config", "-c", help="Path to config.yaml file."
    ),
    ...
    client_id: str = typer.Option(None, "--client-id", help="Client identifier."),
    client_name: str = typer.Option(None, "--client-name", help="Client name for report titles."),
    no_charts: bool = typer.Option(False, "--no-charts", help="Skip chart rendering."),
    verbose: bool = typer.Option(False, "--verbose", "-v", help="Enable debug logging."),
) -> None:
    ...
    This is separate from `ics-toolkit analyze`, which examines
    ...
    try:
        ...
        overrides["referral"]["data_file"] = str(data_file)
        if output_dir is not None:
            overrides["referral"]["output_dir"] = str(output_dir)
        if client_id is not None:
        ...
        logger.info(
            "Client: %s (%s)", referral_settings.client_name, referral_settings.client_id
        )
        logger.info("Data: %s", referral_settings.data_file)
        logger.info("Output: %s", referral_settings.output_dir)

        from ics_toolkit.referral.pipeline import export_outputs, run_pipeline

        result = run_pipeline(referral_settings, skip_charts=no_charts)
